...
int main() {
    ...
    switch(point){
        ...
        case 2: {
            auto best_point = brute_force(ackley_f, ackley_generator);
            std::cout << "best x = " << best_point << std::endl;
            ...
            break;
        case 3: {
            auto best_point = brute_force(matyas_f, matyas_generator);
            std::cout << "best x = " << best_point << std::endl;
        ...
        case 4: {
            auto best_point = brute_force(levi_f, levi_generator);
            std::cout << "best x = " << best_point << std::endl;
            auto levi_f_v = [](domain_t x) {
                return sin(3 * M_PI * x[0]) * sin(3 * M_PI * x[0]) + (x[0] - 1) * (x[0] - 1) * (1 + sin(3 * M_PI * x[0]) * sin(3 * M_PI * x[0]));
            };
            auto best2 = hill_climbing(levi_f_v, {-10}, {10}, 10000);
            std::cout << "best x = " << best2[0] << std::endl;
            break;
        }
        default:
            std::cout << "point default" << std::endl;
            break;
    }
    return 0;
}